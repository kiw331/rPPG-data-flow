...
class CameraThread(QThread):
    ...
    def stop_recording(self):
        ...
        if self.save_dir and self.camera is not None:
            # 💡 Raw 버전은 frames 폴더가 없으므로 save_dir 위치에 직접 저장
            settings_path = os.path.join(self.save_dir, "camera_all_settings.txt")
            temp_pfs_path = os.path.join(self.save_dir, "temp_settings.pfs")
            ...
            try:
                ...
                summary = {
                    "Program_Version": "SingleRaw", # 💡 [버전 명시]
