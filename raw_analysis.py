...
class AnalysisWorker(QThread):
    ...
    def __init__(self, folder_path, start_frame_idx, duration_sec, rois_info, bg_roi_info, bpm_min, bpm_max, raw_w=1024, raw_h=1024, raw_fps=60.0, pixel_format="BayerRG12"):
        super().__init__()
        self.folder_path = folder_path
        self.start_frame_idx = start_frame_idx
        self.duration_sec = duration_sec
        self.rois_info = rois_info
        self.bg_roi_info = bg_roi_info
    ...
    def run(self):
        try:
            ...
            raw_files = sorted(glob.glob(os.path.join(self.folder_path, "*.raw")))
            if not raw_files:
                raise Exception(".raw 파일이 없습니다.")
            
            if os.path.exists(cam_csv_path):
                df_cam = pd.read_csv(cam_csv_path)
            else:
                df_cam = pd.DataFrame({
                    "Frame_Index": np.arange(len(raw_files)),
                    "Timestamp": np.arange(len(raw_files)) / self.raw_fps
                })
            ...
            for idx, (index, row) in enumerate(df_cam_cut.iterrows()):
                ...
                # f_path 찾기
                f_path = os.path.join(frames_dir, f"*_{f_idx:04d}.raw")
                matching = glob.glob(f_path)
                if not matching:
                    f_path = os.path.join(frames_dir, f"{f_idx:04d}.raw")
                    matching = glob.glob(f_path)
                    if not matching:
                        if f_idx < len(raw_files):
                            f_path = raw_files[f_idx]
                        else:
                            continue
                    else:
                        f_path = matching[0]
...
class HistPopup(QDialog):
    def __init__(self, parent=None, frames_dir="", df_cam=None, fps=30.0, current_f_idx=0, min_f_idx=0, max_f_idx=0, raw_w=1024, raw_h=1024):
        super().__init__(parent)
    ...
    def on_raw_setting_changed(self):
        self.raw_w = self.spin_raw_w.value()
        self.raw_h = self.spin_raw_h.value()
        self.fps = self.spin_raw_fps.value()
        if self.folder_path:
            self.show_frame(self.current_frame_idx)
    ...
    def show_frame(self, f_idx):
        ...
        f_path = os.path.join(self.frames_dir, f"*_{f_idx:04d}.raw")
        matching = glob.glob(f_path)
        if not matching:
            f_path = os.path.join(self.frames_dir, f"{f_idx:04d}.raw")
            matching = glob.glob(f_path)
            if not matching:
                raw_files = sorted(glob.glob(os.path.join(self.frames_dir, "*.raw")))
                if f_idx < len(raw_files):
                    f_path = raw_files[f_idx]
                else:
                    return
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("rPPG Raw Analysis Tool")
    ...
    def setup_ui(self):
        ...
        raw_layout.addWidget(QLabel("Format:"))
        self.combo_pixel_format = QComboBox()
        self.combo_pixel_format.addItems([
            "BayerRG12", "BayerGB12", "BayerGR12", "BayerBG12", "Mono12"
        ])
        self.combo_pixel_format.setCurrentText("BayerRG12")
        raw_layout.addWidget(self.combo_pixel_format)

        raw_layout.addWidget(QLabel("W:"))
        self.spin_raw_w = QSpinBox()
        self.spin_raw_w.setRange(1, 9999)
        self.spin_raw_w.setValue(1024)
        self.spin_raw_w.valueChanged.connect(self.on_raw_setting_changed)
        raw_layout.addWidget(self.spin_raw_w)

        raw_layout.addWidget(QLabel("H:"))
        self.spin_raw_h = QSpinBox()
        self.spin_raw_h.setRange(1, 9999)
        self.spin_raw_h.setValue(1024)
        self.spin_raw_h.valueChanged.connect(self.on_raw_setting_changed)
        raw_layout.addWidget(self.spin_raw_h)

        raw_layout.addWidget(QLabel("FPS:"))
        self.spin_raw_fps = QDoubleSpinBox()
        self.spin_raw_fps.setRange(1.0, 1000.0)
        self.spin_raw_fps.setValue(60.0)
        self.spin_raw_fps.valueChanged.connect(self.on_raw_setting_changed)
        raw_layout.addWidget(self.spin_raw_fps)
    ...
    def load_folder(self, folder):
        ...
        raw_files = sorted(glob.glob(os.path.join(self.frames_dir, "*.raw")))
        if not raw_files:
            QMessageBox.warning(self, "경고", ".raw 파일을 찾을 수 없습니다.")
            return
        ...
        self.raw_h = self.spin_raw_h.value()
        self.fps = self.spin_raw_fps.value()
        self.pixel_format = self.combo_pixel_format.currentText()

        # 타임스탬프 자동 생성 (FPS 기반 등간격)
        n_frames = len(raw_files)
        self.df_cam = pd.DataFrame({
            "Frame_Index": np.arange(n_frames),
            "Timestamp": np.arange(n_frames) / self.fps
        })
        self.raw_files = raw_files  # 파일 경로 목록 보관

        folder_name = os.path.basename(self.folder_path)
        duration_sec = (n_frames - 1) / self.fps if self.fps > 0 else 0
        self.lbl_info_folder.setText(f"데이터셋: {folder_name}")
        self.lbl_info_duration.setText(f"Record_Duration: {duration_sec:.1f}s ({n_frames} frames)")
    ...
    # load_camera_settings() 제거됨 - 카메라 설정은 UI에서 수동 입력
    ...
    def start_analysis(self):
        ...
        self.worker = AnalysisWorker(
            ...
            rois_info, bg_roi_info,
            self.spin_bpm_min.value(), self.spin_bpm_max.value(),
            self.raw_w, self.raw_h, self.fps,
            self.pixel_format
        )
