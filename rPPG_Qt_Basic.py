# 파일명: rPPG_Qt_Basic.py
# 지시사항: 
# 1. ROI 및 rPPG 채널 모니터링 연산 기능 완전 제거 (순수 Basic 기능으로 회귀)
# 2. 좌측 패널에 비디오 프리뷰 단독 배치 및 크기 극대화 (Expanding 적용)
# 3. 우측 패널에 하드웨어 센서 그래프 1개와 모든 제어(카메라/녹화/센서) UI를 압축하여 일괄 배치
# 4. 이전의 JSON 설정 요약 저장, 실제 녹화 소요시간(초) 기록, 토글형 조기 종료 기능 모두 유지
...
class CameraThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.camera.UserSetSelector.SetValue("UserSet2")
    ...
    def stop_recording(self):
        ...
        if self.save_dir and self.camera is not None:
            ...
            try:
                ...
                summary = {
                    "Program_Version": "Basic",  # 💡 [여기에 추가] 프로그램 버전 기록
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("rPPG_Qt_Basic (Video Optimized)")
    ...
    def init_ui(self):
        ...
        self.plot_sensor.setMinimumHeight(220) # 단일 그래프이므로 높이를 충분히 확보
        ...
        # 💡 좌측 비디오를 훨씬 크게(stretch=6), 우측 컨트롤 패널을 적당하게(stretch=4) 비율 분배
